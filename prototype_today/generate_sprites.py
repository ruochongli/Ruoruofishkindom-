...
def draw_female_portrait():
    ...
    # 下巴尖
    dr(d, 56, 68, 16, 2, s); dr(d, 60, 70, 8, 2, s)

    # ===== 萌系大眼（参考模板02风格）=====
    ey = 32
    # --- 左眼 ---
    # 上眼线 - 粗弧
    dr(d, 48, ey-2, 18, 2, el)
    dr(d, 46, ey, 22, 2, el)
    dr(d, 48, ey+2, 18, 2, ed)          # 上眼睑阴影
    # 眼白
    dr(d, 48, ey+4, 18, 10, (255,245,250))
    # 眼球 - 上深下浅渐变
    dr(d, 50, ey+4, 14, 4, ed)           # 上半深色
    dr(d, 50, ey+8, 14, 4, e)            # 下半主色
    # 瞳孔
    dr(d, 54, ey+6, 6, 4, ed)
    # 高光（一大一小 + 底部反光）
    dr(d, 52, ey+4, 4, 4, (255,255,255))  # 大高光左上
    dr(d, 58, ey+10, 2, 2, (255,255,255)) # 小高光右下
    dr(d, 56, ey+12, 2, 2, (255,200,220)) # 底部粉色反光
    # 下眼睑阴影
    dr(d, 48, ey+14, 18, 2, (240,210,220))
    # 睫毛
    dr(d, 46, ey-2, 2, 2, el); dr(d, 44, ey, 2, 2, el)
    dr(d, 66, ey-2, 2, 2, el)

    # --- 右眼 ---
    dr(d, 70, ey-2, 18, 2, el)
    dr(d, 68, ey, 22, 2, el)
    dr(d, 70, ey+2, 18, 2, ed)
    dr(d, 70, ey+4, 18, 10, (255,245,250))
    dr(d, 72, ey+4, 14, 4, ed)
    dr(d, 72, ey+8, 14, 4, e)
    dr(d, 76, ey+6, 6, 4, ed)
    dr(d, 74, ey+4, 4, 4, (255,255,255))
    dr(d, 80, ey+10, 2, 2, (255,255,255))
    dr(d, 78, ey+12, 2, 2, (255,200,220))
    dr(d, 70, ey+14, 18, 2, (240,210,220))
    dr(d, 68, ey-2, 2, 2, el); dr(d, 88, ey, 2, 2, el)
    dr(d, 88, ey-2, 2, 2, el)

    # ===== 细弯眉 =====
    dr(d, 52, 24, 4, 2, hd); dr(d, 56, 22, 8, 2, hd); dr(d, 64, 24, 4, 2, hd)
    dr(d, 72, 24, 4, 2, hd); dr(d, 76, 22, 8, 2, hd); dr(d, 84, 24, 4, 2, hd)

    # ===== 小圆嘴（微张）=====
    dr(d, 62, 56, 4, 2, m)
    dr(d, 63, 57, 2, 2, (255,160,170))   # 嘴内粉色

    # ===== 椭圆形腮红 =====
    for x in range(42, 50):
        for y in range(46, 52):
            dot(d, x, y, (255, 160, 170, 130))
